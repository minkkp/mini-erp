...
ERP의 생산·재고 관리 과정에서 발생할 수 있는 동시 요청 문제를 해결하기 위해,  
Spring Boot + JPA 기반에서 트랜잭션 설계를 적용한 미니 ERP 프로젝트입니다.  

낙관적·비관적 락 전략을 비교하고  
k6 부하 테스트로 동작 차이를 검증했습니다.
<br><br>
...
3. 생산 실적 저장
4. 완제품 재고 증가
5. 재고 변경 이력 기록 (StockHistory)

※ 위 과정은 하나의 트랜잭션으로 처리되며, 중간 단계에서 오류 발생 시 전체가 롤백됩니다.
<br><br>
...
정합성이 중요한 도메인 특성상  
자동 재시도보다 실패를 우선하는 전략을 선택했습니다.  

또한 생산 처리 특성상 트랜잭션 대기가 길어질 경우  
시스템 병목이 발생할 수 있다고 판단해,  
비관적 락 대신 낙관적 락을 적용했습니다.
<br><br>

## 📈 동시성 테스트 결과

동일 재고에 대해 부하 테스트를 수행하여  
낙관적 락과 비관적 락의 동작 차이를 비교했습니다.  

초당 500건의 요청을 5초 동안 발생시키는 설정(rate 500 / duration 5s)으로  
부하 환경을 구성했습니다.

- 부하 테스트 도구: k6
- 요청 설정: rate 500 / duration 5s
- 총 요청 규모: 약 2,500건

### 1️⃣ Optimistic Lock (@Version)

- 총 요청: 2,500
- 성공: 1,567 (62.7%)
- 충돌: 933 (37.3%)
- 평균 응답 시간: 약 10ms
<img width="600" height="300" alt="image" src="https://github.com/user-attachments/assets/aa0fe216-d54a-45e2-b9ad-db8e6eae6123" />

### 2️⃣ Pessimistic Lock (PESSIMISTIC_WRITE)

- 총 요청: 2,417
- 성공: 2,417 (100%)
- 충돌: 0건 (0%)
- 평균 응답 시간: 약 700ms
<img width="600" height="300" alt="image" src="https://github.com/user-attachments/assets/02136bdf-e02d-41f5-a850-8fe0a326144e" />
  
모든 요청을 보장했지만, 락 대기로 인해 응답 시간 증가.
### 📌 비교 정리
...
| 성공률 | 62.7% | 100% |
| 실패율 | 37.3% | 0% |
| 평균 응답 시간 | 약 10ms | 약 700ms |
| 처리 전략 | 충돌 시 실패 | 선점 후 대기 |

낙관적 락은 빠르지만 일부 충돌이 발생했고,  
비관적 락은 안정적이지만 응답 시간이 크게 증가했습니다.

트랜잭션 대기로 인한 처리 지연을 고려해 낙관적 락을 선택했습니다.
<br><br>
...
본 프로젝트에서는 생산 처리와 재고 변경 과정에서 발생하는 문제를 직접 정의하고,  
이를 트랜잭션과 도메인 규칙으로 제어하는 구조를 설계·구현했습니다.  
동시 요청 상황에서 발생할 수 있는 데이터 불일치를 실제로 고려하며,  
정합성이 중요한 도메인에서의 트랜잭션 설계 기준을 정리할 수 있었습니다.
