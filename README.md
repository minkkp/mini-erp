...
생산 및 재고 관리 도메인에서 발생하는 **동시성 문제와 트랜잭션 정합성**을  
**Spring Boot + JPA** 기반으로 해결하는 **백엔드 중심 ERP 미니 프로젝트**입니다.
...
- 생산 처리 과정에서 발생할 수 있는 **재고 불일치 문제**를  
  트랜잭션과 도메인 규칙으로 해결
...
생산 실적 등록 시 다음 작업을 **하나의 트랜잭션**으로 처리
...
- 재고 차감은 **Stock 엔티티 내부 로직**에서 검증
- 잘못된 상태가 DB에 저장되지 않도록  
  **도메인 레벨에서 차단**
...
재고는 **현재 상태만 유지**하고,  
모든 변경 사항은 **이력 테이블에 누적 저장**하는 구조입니다.
...
운영 환경에서는 **페이징 또는 파티셔닝 확장 가능성**을 고려했습니다.
...
백엔드 API 흐름과 **업무 시나리오 검증**을 목적으로 구현했습니다.
...
왜 중요한지 직접 경험**할 수 있었습니다.

단순 CRUD 구현을 넘어  
**데이터가 틀리면 안 되는 도메인에서  
백엔드가 책임져야 할 역할**에 집중한 프로젝트입니다.
